...
/* ── PDF.js single-page HTML builder ─────────────────────────────
   Renders ONLY the requested page number using PDF.js (CDN).
   The outer viewport has user-scalable=no so the surrounding app
   does not zoom; the canvas wrapper uses touch-action:pinch-zoom
   so only the PDF responds to a 2-finger pinch. */
...
<body>
  ...
  <script>
    ...
    /* ── Pinch-to-zoom (touch) — only the canvas wrapper scales ── */
...
const PdfCard = ({ pageNumber, pdfUri }: { pageNumber: number; pdfUri: string | null }) => {
  ...
  /* ── Web: iframe with srcDoc so PDF.js runs inside it ──────────── */
  if (Platform.OS === 'web') {
    return (
  ...
  /* ── Native (iOS / Android): WebView with same HTML ─────────────── */
  return (
    <WebView
      originWhitelist={['*']}
      source={{ html }}
